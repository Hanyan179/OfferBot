...
allowed-tools: [get_user_profile, get_memory, sync_jobs, query_jobs, fetch_job_detail, platform_deliver, get_stats]
---
...
本场景涉及 12 个工具，按用途分为 4 组。`allowed-tools` 已全部列出。
...
| 工具名 | 用途 | 关键参数 | 使用步骤 |
|--------|------|---------|---------|
| `浏览器_service_manage` | 检查/启动/停止 浏览器自动化 | `action`: "check"/"start"/"stop" | Step 2 |
| `sync_jobs` | 从 浏览器 同步岗位到本地 DB | `platform`: "liepin" | Step 4 |
...
**等用户在 ActionCard 上确认。** 确认后系统自动执行 `sync_jobs`，TaskPanel 显示进度。
