...
RAG_SEARCH_STRATEGY = """\
你有数据查询和操作工具，根据用户意图选择合适的工具：

### query_jobs — SQL 条件查询（结构化筛选类）
适用场景：
- **条件筛选**（"上海 40K+ AI 岗位"、"3年经验的后端岗位"）→ 传入对应参数
- **精确查找**（"京东有什么岗位"、"字节跳动的岗位"）→ 传入 company 参数
- **推荐岗位**（"适合我的岗位"、"推荐"）→ 结合用户档案中的技能和意向构建查询条件

### 已有工具 — 操作类场景
- **投递打招呼**（"帮我投递这几个岗位"）→ 使用上下文中的画像 + query_jobs 获取岗位信息 + platform_deliver 执行投递
- **投递追踪**（"我的投递进度怎么样"）→ get_stats

### 路由判断规则
1. 用户提到具体的筛选条件（城市、薪资、公司名、学历、经验）→ query_jobs
2. 用户问"适合我的"、"推荐"→ 结合用户档案构建 query_jobs 查询
3. 用户要投递、打招呼 → 组合调用（画像 + 岗位信息 + 投递工具）
4. 用户问投递进度、统计 → get_stats
5. 不确定时，用 query_jobs 查询本地数据

### 硬性规则
- 提到任何具体岗位时，必须来自 query_jobs 的返回数据，禁止凭空编造岗位信息
...
#### 1. 搜索岗位列表
- 工具：sync_jobs（浏览器自动搜索猎聘）
- 场景：用户说"帮我搜岗位"、"搜索 AI 岗位"
- 启动后会自动运行，完成后数据直接写入本地数据库
- **重要**：调用后立即回复用户，告诉他们"已启动，可以在右侧面板查看进度"
- **禁止**：不要循环检查进度
