...
# ---- 猎聘选项（对齐 getjob /api/liepin/config 返回的 options） ----
...
async def on_message(message: cl.Message):
    ...
    await cl.Message(content=f"未知命令: `{text}`\n\n可用: start_task / fetch_detail / deliver / all").send()
...
async def on_action_submit(action: cl.Action):
    ...
    if element:
        ...
        element.props["result_message"] = "模拟执行成功！"
